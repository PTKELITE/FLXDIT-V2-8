...
$(which python3) -m gdown -q IDGDRIVE -O "FLXDIT v2-8.png"
fi

# 8. Download de JSON na Pasta JSON
echo "-> Baixando pacote de JSON FLXDIT v2-8 da pasta JSON..."
mkdir -p /workspace/runpod-slim/ComfyUI/input && \
cd /tmp && \
rm -rf FLXDIT-TEMP && \
git clone --depth 1 https://github.com/PTKELITE/FLXDIT-V2-8.git FLXDIT-TEMP -q && \
cp -a FLXDIT-TEMP/WORKFLOWS/JSON/. /workspace/runpod-slim/ComfyUI/user/default/workflows/ && \
rm -rf FLXDIT-TEMP
check_status "Download das JSON da pasta JSON"


# 8. Download de Imagens na Pasta Input
echo "-> Baixando pacote de imagens FLXDIT v2-8 da pasta IMG-VID..."
mkdir -p /workspace/runpod-slim/ComfyUI/input && \
cd /tmp && \
rm -rf FLXDIT-TEMP && \
git clone --depth 1 https://github.com/PTKELITE/FLXDIT-V2-8.git FLXDIT-TEMP -q && \
cp -a FLXDIT-TEMP/WORKFLOWS/IMG-VID/. /workspace/runpod-slim/ComfyUI/input/ && \
rm -rf FLXDIT-TEMP
check_status "Download das imagens da pasta IMG-VID"
...
#loras LTX hf
wget -q --show-progress --header="Authorization: Bearer HFKEYS" -O /workspace/runpod-slim/ComfyUI/models/loras/LTX2-3-gemma-3-12b-it-abliterated_lora_rank64_bf16.safetensors "https://huggingface.co/Comfy-Org/ltx-2/resolve/main/split_files/loras/gemma-3-12b-it-abliterated_lora_rank64_bf16.safetensors" && \
wget -q --show-progress --header="Authorization: Bearer HFKEYS" -O /workspace/runpod-slim/ComfyUI/models/loras/LTX2-3-_22b_distilled_1.1_lora_dynamic_fro09_avg_rank_111_bf16.safetensors "https://huggingface.co/Comfy-Org/ltx-2.3/resolve/main/split_files/loras/ltx_2.3_22b_distilled_1.1_lora_dynamic_fro09_avg_rank_111_bf16.safetensors" && \
...
#checkpoints civitai
wget -O /workspace/runpod-slim/ComfyUI/models/checkpoints/LTX2-3-sulphur2Base_dev.safetensors "https://civitai.red/api/download/models/2921800?fileId=2802695&token=CIVITAIKEYS" && \
wget -O /workspace/runpod-slim/ComfyUI/models/unet/LTX2-3-DISTILLEDGGUF_q5km.gguf "https://civitai.com/api/download/models/2800384?fileId=2686486&token=CIVITAIKEYS" && \


#LoRAs LTX civitai 
#ADD "LTX2-3-" FRONT NAME TO IDENTIFICACION 
wget -O /workspace/runpod-slim/ComfyUI/models/loras/LTX2-3-Crisp_Enhance.safetensors "https://civitai.red/api/download/models/2849716?fileId=2735885&token=CIVITAIKEYS" && \
...
wget --header="Authorization: Bearer HFKEYS" \
...
wget -O /workspace/runpod-slim/ComfyUI/models/loras/KLEIN-Unchained-V2.safetensors "https://civitai.red/api/download/models/2753532?fileId=2639950&token=CIVITAIKEYS" && \

#wget -O /workspace/runpod-slim/ComfyUI/models/loras/klein-deepthroat-v2-5epoc-k3nk.safetensors "https://civitai.red/api/download/models/2832958?fileId=2719117&token=CIVITAIKEYS" #&& \
